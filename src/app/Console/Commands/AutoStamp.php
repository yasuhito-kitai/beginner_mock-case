...
//退勤打刻をしていないユーザーの精算（上記で処理した休憩し続けているユーザーも含む）

    //（定義）退勤：日を跨いだ時点で退勤打刻がされていないレコード抽出（休憩の有無関係なく）
    $auto_update_items_records = Item::where("date", "=", "$yesterday")->whereNull("clock_out")->get();

            // 当該ユーザーの休憩の合計を計算・登録（休憩がなければ00:00:00で登録）
            foreach($auto_update_items_records as $auto_update_items_record){
        //break_time_totalを計算
            $break_time_total_raw = BreakTime::where("item_id", "=", "$auto_update_items_record->id")->selectRaw('SUM(TIME_TO_SEC(break_time)) as break_time_sum')->first();/* 秒に直す*/
