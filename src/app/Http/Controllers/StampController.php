...
{   // ホーム（打刻）画面
    ...
    {   // ボタン活性・非活性
        ...
        if($clock_in_status !== null && $break_end_possible == null ) { /*nullでない（レコード取得している）＝出勤している＆休憩していない⇒勤務終了、休憩開始が活性*/
        ...
        }elseif($clock_in_status !== null && $break_end_possible !== null) { /*出勤している＆休憩している⇒休憩終了のみが活性*/
            $disabled_clock_in = "disabled";
        ...
        }elseif($clock_in_status == null || $clock_out_status !== null) { /*出勤前or退勤後⇒勤務開始のみが活性*/
            $disabled_clock_in = "";
            $disabled_clock_out = "disabled";
            $disabled_break_start = "disabled";
            $disabled_break_end = "disabled";
    ...
    public function clock_out()
    {
        ...
        $update_subject_id=Item::select("id")->where("user_id", "=", $user_id)->where("date", "=", $today)->orderBy('updated_at', 'desc')->first();/*勤務時間の計算対象レコードは「ログイン中のユーザー」が「今日」「直近更新（勤務終了）」した主キーを持つレコード*/
    ...
    {
        //全ユーザー情報を取得
        $user_records = User::select("id","name", "email")->paginate(5);
    ...
    {
        $user_records = User::with('item')->KeywordSearch($request->keyword)->paginate(5);
